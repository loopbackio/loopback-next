...
import {BuildContextArgs, CreateApp} from '@graphql-ez/express';
import {
  Binding,
...
export class GraphQLServer extends Context implements Server {
  ...
  async start() {
    ...
    const graphqlContextResolver: ContextFunction =
      (await this.get(GraphQLBindings.GRAPHQL_CONTEXT_RESOLVER, {
        optional: true,
      })) ?? ((context: BuildContextArgs) => context as {});

    const graphQLServer = CreateApp({
      buildContext: graphqlContextResolver,
      schema,
      ...this.options.graphql,
    });

    await graphQLServer.buildApp({
      app: this.expressApp,
