...
object| |`rest.http.request.context`|`Http.CONTEXT`|`Context`|Request level
context| |`rest.http.response`|`Http.RESPONSE`|`ServerResponse`|The raw `http`
response object| |`routes.${route.verb}.${route.path}`||`RouteEntry`|Route entry
specified in api-spec| |`rest.sequence`|`SEQUENCE`|`SequenceHandler`|Class that
