  ...
  function createContext() {
    ...
    for (const e in strategies) {
      ctx
        ...
        .inScope(BindingScope.SINGLETON)
        .tag({extensionPoint: AUTH_EXTENSION_POINT, name: e});
...
describe('Application support for extension points', () => {
  ...
  it('registers an extension point by class name', () => {
    ...
    expect(binding.tagMap).to.containEql({
      name: 'extensionPoints.AuthenticationManager',
    });
    expect(binding.tagNames).to.containEql('extensionPoint');
  });
  ...
  it('registers an extension by class name', () => {
    ...
    expect(binding.tagMap).to.containEql({
      name: 'LocalStrategy',
      extensionPoint: AUTH_EXTENSION_POINT,
    });
  });
