...
import {expect, toJSON, skipIf} from '@loopback/testlab';
      ...
      () => {
        describe('HasMany inclusion resolvers - acceptance', () => {
            ...
            withCrudCtx(async function setupRepository(ctx: CrudTestContext) {
                ...
                expect(customerRepo.customers.inclusionResolver).to.not.be.undefined();
            ...
            it("throws an error if the repository doesn't have such relation names", async () => {
                ...
                await expect(customerRepo.find({include: [{relation: 'managers'}]})
                ).to.be.rejectedWith(
            ...
            it('throws error if the target repository does not have the registered resolver', async () => {
                ...
                await expect(customerRepo.find({include: [{relation: 'orders'}]})
                ).to.be.rejectedWith(
              ...
              it('returns an array of related instances via find() method for one target instance', async () => {
                ...
                const result = await customerRepo.find({include: [{relation: 'orders'}]});
