...
module.exports = class ControllerRelation extends ArtifactGenerator {
    ...
    generateRelationController(sourceModel, targetModel, foreignKey, relationName) {

        let project = new ast.Project();
        const sourceFile = this.addFileToProject(project, (this.artifactInfo.modelDir + "/" + this.options.sourceModel + ".model.ts"));
        ...
        this.artifactInfo.srcModel = utils.camelCase(
            sourceModel,
        );
        this.artifactInfo.dstModel = utils.camelCase(
            targetModel,
        );
        this.artifactInfo.modelName = utils.toClassName(
            targetModel,
        );
        ...
        this.artifactInfo.name = utils.camelCase(
    ...
    getKeyType(sourceFile, propertyName) {
        ...
        if (classObj.getProperties().map(x => x.getName()).includes(propertyName)) {
